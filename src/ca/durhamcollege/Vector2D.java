/*
Name: Ashok Sasitharan
Student Number: 100745484
Date: November 18 2020

 */
package ca.durhamcollege;

import org.jetbrains.annotations.NotNull;

public class Vector2D
{


    // PRIVATE INSTANCE VARIABLES
    private float x;
    private float y;

    //PUBLIC PROPERTIES (MUTATORS AND ACCESSORS)
    public float getX()
    {
        return x;
    }

    public void setX(final float x)
    {
        this.x = x;
    }

    public float getY()
    {
        return y;
    }

    public void setY(final float y)
    {
        this.y = y;
    }

    public void set(final float x, final float y)
    {
        this.x = x;
        this.y = y;
    }
    public void set( final Vector2D vector)
    {
        this.x = x;
        this.y = y;
    }
    //CONSTRUCTORS

    //default constructor
    Vector2D()
    {
        this.x = 0;
        this.y = 0;
    }
//parameterized constructor
    Vector2D(final float x, final float y)
    {
        set(x,y);
    }
//parameterized constructor with Vector2D parameter
    Vector2D(final Vector2D vector)
    {
        set(vector.getX(),vector.getY());
    }

    //PRIVATE METHODS

    // PUBLIC METHODS

    /**
     * This function adds the current Vector with the inputted vector
     * @param rhs
     */
    public void add(final Vector2D rhs)
    {
        this.setX(this.getX() + rhs.getX());
        this.setY(this.getY() + rhs.getY());
    }

    /**
     * This function subtracts the current vector with the inputted vector
     * @param rhs
     */
    public void subtract(final Vector2D rhs)
    {
        this.setX(this.getX() - rhs.getX());
        this.setY(this.getY() - rhs.getY());
    }

    /**
     * This function multiplies the current vector with the inputted vector
     * @param rhs
     */
    public void multiply(final Vector2D rhs)
    {
        this.setX(this.getX() * rhs.getX());
        this.setY(this.getY() * rhs.getY());
    }

    /**
     * This function divides the current vector with the inputted vector
     * @param rhs
     */
    public void divide(final Vector2D rhs)
    {
        this.setX(this.getX() / rhs.getX());
        this.setY(this.getY() / rhs.getY());
    }

    /**
     * this function returns true if the inputted Vector2D's are equal to each other
     * @param rhs
     * @return
     */
    public boolean equals(Vector2D rhs)
    {
       return ((getX()== rhs.getX()) && (getY() == rhs.getY()));
    }
    /**
     * this function returns true if the inputted Vector2D's are not equal to each other
     * @param rhs
     * @return
     */
    public boolean notEquals(Vector2D rhs)
    {
        return ((getX() != rhs.getX()) && (getY() != rhs.getY()));
    }

    /**
     * This function gets the magnitude of the current Vector
     * @return
     */
    public float getMagnitude()
    {
      return (float) (Math.sqrt(this.getX() * this.getX() + this.getY() * this.getY()));
    }

    /**
     * This function gets the magnitude of the current vector without square rooting it
     * @return
     */
    public float getSqrMagnitude()
    {
        return (this.getX() * this.getX() + this.getY() * this.getY());
    }

    /**
     * This function scales the current vector with the inputted scale
     * @param scale
     */
    public void setScaled(float scale)
    {
        this.set(this.getX()*scale, this.getY() * scale);

    }

    /**
     * This function scales the current Vector2D with an inputted Vector2D
     * @param scale
     */
    public void setScaled(Vector2D scale)
    {
        this.set(this.getX()*scale.x, this.getY() * scale.y);

    }

    /**
     * This function normalizes the vector
     */
    public void normalize()
    {
        final var magnitude = this.getMagnitude();
        if((magnitude) > 9.99999974737875E-06)
        {
            set(getX() / magnitude, getY() /magnitude);
        }
        else
        {
            set(Vector2D.zero());
        }
    }

    /**
     * This function gets the normalized vector
     * @return
     */
    public Vector2D getNormalized()
    {
        Vector2D vector = new Vector2D(getX(),getY());
        vector.normalize();
        return vector;

    }

    /**
     * Overrided to string function
     * @return
     */
    @Override
    public String toString()
    {
        return "(" + x + " , " + y + ")";
    }

    //STATIC METHODS

    /**
     * This function returns a new vector defaulted to (0,0)
     * @return
     */
    public static Vector2D zero()
    {
        return new Vector2D(0.0f,0.0f);
    }

    /**
     * This function returns a new vector defaulted to (1,1)
     * @return
     */
    public static Vector2D one()
    {
        return new Vector2D(1.0f,1.0f);
    }

    /**
     * This function returns a new vector defaulted to (-1,0)
     * @return
     */
    public static Vector2D left()
    {
        return new Vector2D(-1.0f,0.0f);
    }

    /**
     * This function returns a new vector defaulted to (1,0)
     * @return
     */
    public static Vector2D right()
    {
        return new Vector2D(1.0f,0.0f);
    }

    /**
     * This function returns a new vector defaulted to (0,1)
     * @return
     */
    public static Vector2D up()
    {
        return new Vector2D(0.0f,1.0f);
    }

    /**
     * This function returns a new vector defaulted to (0,-1)
     * @return
     */
    public static Vector2D down()
    {
        return new Vector2D(0.0f,-1.0f);
    }


    public static Vector2D lerp(final Vector2D a, final Vector2D b, float t)
    {
        //clamp t between 0.0 and 1.0
        if((double)(t) < 0.0)
        {
            t=0.0f;
        }
        if((double)(t) > 1.0)
        {
            t=1.0f;
        }
        return new Vector2D(a.getX() +( b.getX() - a.getX()) *t, a.getY() + (b.getY() -a.getY()) +t);

    }

    public static float dot (final Vector2D lhs, final Vector2D rhs)
    {
        return (float)((double)(lhs.getX()) * (double)(rhs.getX()) + (double)(lhs.getY()) * (double)(rhs.getY()));
    }

    /**
     * This function gets the distance between 2 Vector 2D objects
     * @param a
     * @param b
     * @return
     */
    public static float distance(final Vector2D a, final Vector2D b)
    {
        final var  delta_x = (double)(b.getX()) - (double)(a.getX());
        final var  delta_y = (double)(b.getY()) - (double)(a.getY());
        return (float)(Math.sqrt(delta_x*delta_x + delta_y * delta_y));
    }
}
